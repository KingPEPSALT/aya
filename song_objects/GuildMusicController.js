...
module.exports = class GuildMusicController{
    ...
    async playTop(){
        ...
        const dispatcher = this.voice.connection.play(await ytdl(this.queue[0].url).catch(e=>{
            return 'Invalid URL.';
    ...
    currentElapsedTime(){
        if(!this.nowPlaying()) return 'Nothing is playing.';
        return this.voice.connection.dispatcher.streamTime - this.voice.connection.dispatcher.pausedTime;
    }

    displayQueue(){
        if(this.queue.length == 0) return 'The queue is empty.';
        return this.queue.map((song, position)=>`${position+1} | ${song.title} - ${song.channel} | Length: ${song.formattedLength()}`).join('\n');
    }
